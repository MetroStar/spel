...
##############################################################################
PROGNAME="$(basename "$0")"
AMIGENBOOTDEVMULT="${CHIMERA_AMIGENBOOTDEVMULT:-1.2}"
AMIGENBOOTDEVSZ="${CHIMERA_AMIGENBOOTDEVSZ:-1024}"
AMIGENBOOTSIZE="${CHIMERA_AMIGENBOOTSIZE}"
AMIGENBRANCH="${CHIMERA_AMIGENBRANCH:-master}"
AMIGENCHROOT="${CHIMERA_AMIGENCHROOT:-/mnt/ec2-root}"
AMIGENCROSSDISTRO="${CHIMERA_AMIGENCROSSDISTRO:-false}"
AMIGENNOSIGNATURE="${CHIMERA_AMIGENNOSIGNATURE:-false}"
AMIGENFSTYPE="${CHIMERA_AMIGENFSTYPE:-xfs}"
AMIGENICNCTURL="${CHIMERA_AMIGENICNCTURL}"
AMIGENMANFST="${CHIMERA_AMIGENMANFST}"
AMIGENPKGGRP="${CHIMERA_AMIGENPKGGRP:-core}"
AMIGENREPOS="${CHIMERA_AMIGENREPOS}"
AMIGENREPOSRC="${CHIMERA_AMIGENREPOSRC}"
AMIGENROOTNM="${CHIMERA_AMIGENROOTNM}"
AMIGENSOURCE="${CHIMERA_AMIGEN8SOURCE:-file://$(dirname $(dirname $(dirname $(readlink -f "$0"))))/vendor/amigen8}"
AMIGENSSMAGENT="${CHIMERA_AMIGENSSMAGENT}"
AMIGENSTORLAY="${CHIMERA_AMIGENSTORLAY}"
AMIGENTIMEZONE="${CHIMERA_TIMEZONE:-UTC}"
AMIGENVGNAME="${CHIMERA_AMIGENVGNAME}"
AWSCFNBOOTSTRAP="${CHIMERA_AWSCFNBOOTSTRAP}"
AWSCLIV1SOURCE="${CHIMERA_AWSCLIV1SOURCE}"
AWSCLIV2SOURCE="${CHIMERA_AWSCLIV2SOURCE:-https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip}"
CLOUDPROVIDER="${CHIMERA_CLOUDPROVIDER:-aws}"
EPELRELEASE="${CHIMERA_EPELRELEASE}"
EPELREPO="${CHIMERA_EPELREPO}"
EXTRARPMS="${CHIMERA_EXTRARPMS}"
FIPSDISABLE="${CHIMERA_FIPSDISABLE}"
GRUBTMOUT="${CHIMERA_GRUBTMOUT:-5}"
HTTP_PROXY="${CHIMERA_HTTP_PROXY}"
USEDEFAULTREPOS="${CHIMERA_USEDEFAULTREPOS:-true}"
USEROOTDEVICE="${CHIMERA_USEROOTDEVICE:-true}"
AMIGENSSLVERIFY="${CHIMERA_AMIGENSSLVERIFY:-true}"


read -r -a BUILDDEPS <<< "${CHIMERA_BUILDDEPS:-lvm2 yum-utils unzip git}"

ELBUILD="/tmp/el-build"

# Debug: Show air-gapped build configuration
echo "=== Air-Gapped Build Configuration ==="
echo "CHIMERA_AMIGENCROSSDISTRO=${CHIMERA_AMIGENCROSSDISTRO:-not set}"
echo "AMIGENCROSSDISTRO=${AMIGENCROSSDISTRO}"
echo "CHIMERA_AMIGENNOSIGNATURE=${CHIMERA_AMIGENNOSIGNATURE:-not set}"
echo "AMIGENNOSIGNATURE=${AMIGENNOSIGNATURE}"
echo "CHIMERA_USEDEFAULTREPOS=${CHIMERA_USEDEFAULTREPOS:-not set}"
echo "CHIMERA_AMIGENREPOSRC=${CHIMERA_AMIGENREPOSRC:-not set}"
echo "CHIMERA_AMIGENREPOS=${CHIMERA_AMIGENREPOS:-not set}"
echo "CHIMERA_AMIGENSSLVERIFY=${CHIMERA_AMIGENSSLVERIFY:-not set}"
...
function BuildChroot {
    ...
    then
        ...
        # Configure yum.conf/dnf.conf in chroot
        for CONFFILE in "${AMIGENCHROOT}/etc/yum.conf" "${AMIGENCHROOT}/etc/dnf/dnf.conf"
        do
            if [[ -f "${CONFFILE}" ]] && ! grep -q "^sslverify" "${CONFFILE}"
            then
                echo "sslverify=0" >> "${CONFFILE}"
                echo "Added sslverify=0 to ${CONFFILE}"
            fi
        done
...
function PrepBuildDevice {
    ...
    if [[ -n ${CHIMERA_AMIGENBUILDDEV:-} ]]
    then
      AMIGENBUILDDEV="${CHIMERA_AMIGENBUILDDEV}"
    else
...
echo "Checking ${CHIMERA_AMIGENBUILDDEV} for VTOC to nuke..."
if [[ -b "${CHIMERA_AMIGENBUILDDEV}" ]]
then
    echo "${CHIMERA_AMIGENBUILDDEV} is a valid block device. Nuking VTOC... "

    retry 5 dd if=/dev/urandom of="${CHIMERA_AMIGENBUILDDEV}" bs=1024 count=10240
    echo "Cleared."
fi
