...
export PREFIX="spel"                 # resource name prefix
...
KMS_KEY_ID=$(aws kms create-key \
  ...
  --tags TagKey=Name,TagValue="${PREFIX}-ssm-key" \
         ...
         TagKey=Module,TagValue=spel-ssm \
...
aws logs create-log-group \
  ...
  --tags Name="${PREFIX}-ssm-logs",ManagedBy=manual,Module=spel-ssm
...
aws iam create-role \
  ...
  --tags Key=Name,Value="${PREFIX}-ssm-instance-role" \
         Key=ManagedBy,Value=manual Key=Module,Value=spel-ssm
...
aws iam put-role-policy \
  --role-name "${PREFIX}-ssm-dhmc-role" \
  --policy-name "${PREFIX}-ssm-dhmc-spel" \
...
CALLER_POLICY_ARN=$(aws iam create-policy \
  ...
  --description "Permissions for CI runners and operators to invoke SSM operations on SPEL instances" \
...
SPEL AMIs to newly launched EC2 instances, so State Manager associations
...
aws lambda create-function \
  ...
  --tags Name="${PREFIX}-ami-tag-propagation",ManagedBy=manual,Module=spel-ssm
```
...
aws logs put-metric-filter \
  ...
  --metric-transformations \
    metricName="${PREFIX}-SSMErrors",metricNamespace="SPEL/SSM",metricValue=1,defaultValue=0
...
aws logs put-metric-filter \
  ...
  --metric-transformations \
    metricName="${PREFIX}-ComplianceFailures",metricNamespace="SPEL/SSM",metricValue=1,defaultValue=0
...
for VPCE in $(aws ec2 describe-vpc-endpoints \
  --filters "Name=vpc-id,Values=${VPC_ID}" "Name=tag:Module,Values=spel-ssm" \
