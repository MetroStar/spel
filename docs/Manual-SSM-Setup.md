...
export PREFIX="crucible"                 # resource name prefix
...
KMS_KEY_ID=$(aws kms create-key \
  ...
  --tags TagKey=Name,TagValue="${PREFIX}-ssm-key" \
         ...
         TagKey=Module,TagValue=crucible-ssm \
...
aws logs create-log-group \
  ...
  --tags Name="${PREFIX}-ssm-logs",ManagedBy=manual,Module=crucible-ssm
...
aws iam create-role \
  ...
  --tags Key=Name,Value="${PREFIX}-ssm-instance-role" \
         Key=ManagedBy,Value=manual Key=Module,Value=crucible-ssm
...
aws iam put-role-policy \
  --role-name "${PREFIX}-ssm-dhmc-role" \
  --policy-name "${PREFIX}-ssm-dhmc-crucible" \
...
Crucible AMIs to newly launched EC2 instances, so State Manager associations
...
aws lambda create-function \
  ...
  --tags Name="${PREFIX}-ami-tag-propagation",ManagedBy=manual,Module=crucible-ssm
```
...
aws logs put-metric-filter \
  ...
  --metric-transformations \
    metricName="${PREFIX}-SSMErrors",metricNamespace="CRUCIBLE/SSM",metricValue=1,defaultValue=0
...
aws logs put-metric-filter \
  ...
  --metric-transformations \
    metricName="${PREFIX}-ComplianceFailures",metricNamespace="CRUCIBLE/SSM",metricValue=1,defaultValue=0
...
for VPCE in $(aws ec2 describe-vpc-endpoints \
  --filters "Name=vpc-id,Values=${VPC_ID}" "Name=tag:Module,Values=crucible-ssm" \
