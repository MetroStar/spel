...
This document describes STIG (Security Technical Implementation Guide) controls that are intentionally not implemented, require compensating controls, or must be configured post-deployment for Crucible hardened AMIs.

## Overview

Crucible hardened AMIs are built using OpenSCAP with the DISA STIG profile. Due to the nature of cloud-based AMI builds, certain STIG controls cannot be applied at build time or require alternative implementations.
...
These controls are marked "Not Applicable" because Crucible AMIs are server builds without a graphical interface.
...
Crucible AMIs use two complementary compliance scanning tools at build time:
...
`audit_content: git`). For air-gapped environments, set the `crucible_goss_binary_url`
...
| ---------- | ------- | ------------ | -------------------------- |
| 2026-02-02 | 1.0     | Crucible Team | Initial documentation      |
