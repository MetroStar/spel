...
variable "aws_iam_instance_profile" {
  description = "Name of an IAM instance profile to attach to the build instance. Required for SSM-based completion polling on Windows builds. Create using infra-setup.yml or provide your own with SSM permissions."
  type        = string
  default     = ""
}
...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Running RHEL8-STIG playbook (controller=python3.9, remote modules=python3.6 for SELinux)...'",
      "EXTRA_VARS='{\"ansible_python_interpreter\": \"/usr/bin/python3.6\", \"system_is_ec2\": true, \"rhel8stig_copy_existing_zone\": false, \"setup_audit\": true, \"run_audit\": true, \"fetch_audit_output\": true, \"rhel8stig_040136\":false, \"rhel8stig_sudoers_exclude_nopasswd_list\": [\"ec2-user\", \"maintuser\", \"vagrant\", \"ssm-user\"], \"rhel8stig_faillock_exclude_users\": [\"ec2-user\", \"ssm-user\"]}'",
      "if [ -n \"$GOSS_BINARY_URL\" ]; then echo \"Using custom Goss binary URL: $GOSS_BINARY_URL\"; EXTRA_VARS=$(echo $EXTRA_VARS | sed \"s/}$/,\\\"audit_binary\\\": \\\"$GOSS_BINARY_URL\\\", \\\"get_audit_binary_checksum\\\": false}/\"); fi",
      "echo \"EXTRA_VARS: $EXTRA_VARS\"",
      "python3.9 -m ansible playbook -i localhost, -c local $HOME/.ansible/roles/RHEL8-STIG/site.yml -e \"$EXTRA_VARS\"",
      "echo 'Installing OpenSCAP for compliance scanning...'",
      "yum install -y openscap-scanner scap-security-guide",
      "echo 'Running OpenSCAP STIG compliance scan...'",
