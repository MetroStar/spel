...
##############################################################################
PROGNAME="$(basename "$0")"
AMIGENBOOTDEVMULT="${CRUCIBLE_AMIGENBOOTDEVMULT:-1.2}"
AMIGENBOOTDEVSZ="${CRUCIBLE_AMIGENBOOTDEVSZ:-1024}"
AMIGENBOOTSIZE="${CRUCIBLE_AMIGENBOOTSIZE}"
AMIGENBRANCH="${CRUCIBLE_AMIGENBRANCH:-master}"
...
AMIGENSOURCE="${CRUCIBLE_AMIGEN8SOURCE:-file://$(dirname $(dirname $(dirname $(readlink -f "$0"))))/vendor/amigen8}"
...
EPELRELEASE="${CRUCIBLE_EPELRELEASE}"
EPELREPO="${CRUCIBLE_EPELREPO}"
...
read -r -a BUILDDEPS <<< "${CRUCIBLE_BUILDDEPS:-lvm2 yum-utils unzip git}"
...
function PrepBuildDevice {
    ...
    if [[ -n ${CRUCIBLE_AMIGENBUILDDEV:-} ]]
    then
      AMIGENBUILDDEV="${CRUCIBLE_AMIGENBUILDDEV}"
    else
...
echo "Checking ${CRUCIBLE_AMIGENBUILDDEV} for VTOC to nuke..."
if [[ -b "${CRUCIBLE_AMIGENBUILDDEV}" ]]
then
    echo "${CRUCIBLE_AMIGENBUILDDEV} is a valid block device. Nuking VTOC... "

    retry 5 dd if=/dev/urandom of="${CRUCIBLE_AMIGENBUILDDEV}" bs=1024 count=10240
    echo "Cleared."
fi
