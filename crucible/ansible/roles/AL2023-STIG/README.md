...
- **No DISA AL2023 STIG**: DISA has not published an official STIG benchmark for Amazon Linux 2023
- **EL9 Compatibility**: Amazon Linux 2023 is based on Fedora and is largely EL9-compatible
- **RHEL9 STIG Baseline**: Uses RHEL 9 STIG controls as the security baseline
- **AWS STIG Script Fallback**: This role is used when AWS's official STIG script doesn't support AL2023

### Maintenance Strategy

- **One-time snapshot**: No planned upstream sync with ansible-lockdown/RHEL9-STIG
- **Local patches only**: Modifications limited to AL2023 OS detection
- **Minimal changes**: Preserves original RHEL9-STIG logic for auditability

### Key Modifications

1. **OS Detection** (`tasks/main.yml`): Added Amazon Linux 2023 to supported distributions
2. **Variables** (`vars/Amazon.yml`): AL2023-specific settings (e.g., `system_is_ec2: true`)
3. **Benchmark Name** (`defaults/main.yml`): Changed from `RHEL9-STIG` to `AL2023-STIG`

### Usage

```bash
ansible-playbook -i localhost, -c local site.yml -e '{"system_is_ec2": true}'
```

### Disabled Controls (EC2/Firewall)

The following firewall-related controls are disabled by default for EC2:
- `rhel_09_251010` through `rhel_09_251045`

---

## Original RHEL9-STIG Documentation
...
![Private Pull Requests](https://img.shields.io/endpoint?url=https://ansible-lockdown.github.io/github_linux_IaC/badges/Private-RHEL9-STIG/prs.json)
![Private Closed Issues](https://img.shields.io/endpoint?url=https://ansible-lockdown.github.io/github_linux_IaC/badges/Private-RHEL9-STIG/issues-closed.json)

---

## Looking for support? 🤝

[Lockdown Enterprise](https://www.lockdownenterprise.com#GH_AL_RHEL9-STIG)

[Ansible support](https://www.mindpointgroup.com/cybersecurity-products/ansible-counselor#GH_AL_RHEL9-STIG)
