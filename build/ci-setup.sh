...
install_python_deps() {
    ...
    if detect_offline_mode; then
        # Offline mode - install from vendored wheels
        ...
        if [ -d "${REPO_ROOT}/spel/ansible/collections" ]; then
            ...
            if ls "${REPO_ROOT}"/spel/ansible/collections/*.tar.gz 1> /dev/null 2>&1; then
                ...
                for tarball in "${REPO_ROOT}"/spel/ansible/collections/*.tar.gz; do
                    ...
                    # Install collection (allow command to fail without exiting script)
                    set +e
                    ansible-galaxy collection install "$tarball" --force >/dev/null 2>&1
                    INSTALL_EXIT=$?
                    set -e
