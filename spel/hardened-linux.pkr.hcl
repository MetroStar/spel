...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Running Ansible Lockdown'",
      "echo 'Installing Python 3.9 for EL8 (system default is Python 3.6)...'",
      "yum install -y python39 python39-pip python39-devel libselinux-devel policycoreutils-python-utils",
      "python3.9 --version",
      "echo 'Installing SELinux Python bindings for Python 3.9...'",
      "python3.9 -m pip install selinux",
      "echo 'Copying SELinux policy management modules from Python 3.6 to Python 3.9...'",
      "mkdir -p /usr/local/lib64/python3.9/site-packages",
      "echo 'Checking for SELinux modules in Python 3.6...'",
      "ls -la /usr/lib/python3.6/site-packages/ | head -20",
      "echo 'Checking for specific files...'",
      "test -f /usr/lib/python3.6/site-packages/seobject.py && echo 'seobject.py EXISTS' || echo 'seobject.py NOT FOUND'",
      "test -f /usr/lib/python3.6/site-packages/semanage.py && echo 'semanage.py EXISTS' || echo 'semanage.py NOT FOUND'",
      "test -d /usr/lib/python3.6/site-packages/sepolicy && echo 'sepolicy/ EXISTS' || echo 'sepolicy/ NOT FOUND'",
      "echo 'Attempting to copy files...'",
      "cp -rv /usr/lib/python3.6/site-packages/seobject.py /usr/local/lib64/python3.9/site-packages/ || echo 'FAILED to copy seobject.py'",
      "cp -rv /usr/lib/python3.6/site-packages/semanage.py /usr/local/lib64/python3.9/site-packages/ || echo 'FAILED to copy semanage.py'",
      "cp -rv /usr/lib/python3.6/site-packages/sepolicy /usr/local/lib64/python3.9/site-packages/ || echo 'FAILED to copy sepolicy'",
      "echo 'Verifying copied modules...'",
      "ls -la /usr/local/lib64/python3.9/site-packages/ | head -20",
      "echo 'Upgrading pip for EL8 to support newer wheel formats...'",
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "mkdir -p $HOME/.ansible/roles",
      "cp -r /tmp/RHEL8-STIG $HOME/.ansible/roles/",
      "ansible-playbook -i localhost, -c local $HOME/.ansible/roles/RHEL8-STIG/site.yml -e '{\"ansible_python_interpreter\": \"/usr/bin/python3.9\", \"system_is_ec2\": true, \"rhel8stig_copy_existing_zone\": false, \"setup_audit\": true, \"run_audit\": true, \"fetch_audit_output\": true, \"rhel_08_040136\":false}'",
      "bash /tmp/boot-fips-wrapper.sh post",
      "rm -rf /var/lib/cloud/seed/nocloud-net",
      "rm -rf /var/lib/cloud/sem",
      "rm -rf /var/lib/cloud/data",
