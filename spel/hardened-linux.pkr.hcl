...
build {
  ...
  provisioner "powershell" {
    ...
    inline = [
      "# Force TLS 1.2 and set error handling",
      ...
      "} catch { Write-Host \"  - DHCP service config failed: $_\" }",
      "",
      "try {",
      "    Start-Service -Name 'Dhcp' -ErrorAction Stop",
      "    Write-Host '  - DHCP service started'",
      "} catch { Write-Host \"  - DHCP service start failed: $_\" }",
      "",
      "Get-NetAdapter | Where-Object {$_.Status -eq 'Up'} | ForEach-Object {",
      "    try {",
      "        Set-NetIPInterface -InterfaceIndex $_.ifIndex -Dhcp Enabled -ErrorAction Stop",
      "        Write-Host \"  - DHCP enabled on adapter $($_.Name)\"",
      "    } catch { Write-Host \"  - DHCP config failed on $($_.Name): $_\" }",
      "}",
      "",
      ...
      "        Write-Host \"  - $svc set to Automatic\"",
      "    }",
      "}",
      "",
      ...
      "",
      "# ----- STEP 3: EC2Launch v1 Sysprep Prep (creates SetupComplete.cmd) -----",
      "Write-Host 'Step 3: Running EC2Launch v1 InitializeInstance and Sysprep prep...'",
      ...
      "",
      "# ----- STEP 4: APPEND our cleanup commands to EC2Launch's SetupComplete.cmd -----",
      "Write-Host 'Step 4: Appending SPEL cleanup commands to SetupComplete.cmd...'",
      "$setupCmd = 'C:\\Windows\\Setup\\Scripts\\SetupComplete.cmd'",
      "if (Test-Path $setupCmd) {",
      "    Write-Host 'Found existing SetupComplete.cmd, appending SPEL commands...'",
      "} else {",
      "    Write-Host 'Creating new SetupComplete.cmd...'",
      "    New-Item -Path 'C:\\Windows\\Setup\\Scripts' -ItemType Directory -Force | Out-Null",
      "    '@ECHO OFF' | Out-File -FilePath $setupCmd -Encoding ASCII",
      "}",
      "",
      "# Append SPEL cleanup commands",
      "$spelCommands = @(",
      "    ''",
      "    ':: =============================================='",
      "    ':: SPEL Custom Cleanup - Added by Packer build'",
      "    ':: =============================================='",
      "    'echo [%date% %time%] Starting SPEL cleanup >> C:\\SetupComplete.log'",
      "    ''",
      "    ':: Rename Administrator to maintuser'",
      "    'wmic useraccount where \"Name=''Administrator''\" call rename Name=\"maintuser\" >> C:\\SetupComplete.log 2>&1'",
      "    ''",
      "    ':: Delete TempPackerUser'",
      "    'net user TempPackerUser /delete >> C:\\SetupComplete.log 2>&1'",
      "    'rmdir /s /q \"C:\\Users\\TempPackerUser\" >> C:\\SetupComplete.log 2>&1'",
      "    ''",
      "    'echo [%date% %time%] SPEL cleanup finished >> C:\\SetupComplete.log'",
      ")",
      "$spelCommands | Out-File -FilePath $setupCmd -Encoding ASCII -Append",
      "",
      "Write-Host 'SetupComplete.cmd updated with SPEL cleanup commands.'",
      "Write-Host 'Contents of SetupComplete.cmd:'",
      "Get-Content $setupCmd | ForEach-Object { Write-Host \"  $_\" }",
      "",
      "Write-Host 'EC2Launch v1 sysprep prep complete. Packer will stop the instance.'"
    ]
  }
  ...
  provisioner "powershell" {
    ...
    inline = [
      ...
      "& 'C:/Windows/Temp/cleanup-sysprep.ps1' -SkipSysprep -Verbose",
      "",
      "# ----- STEP 3: EC2Launch v2 Reset (clears instance data) -----",
      "Write-Host 'Step 3: Running EC2Launch v2 reset...'",
      "& 'C:/Program Files/Amazon/EC2Launch/ec2launch' reset --block",
      "",
      "# ----- STEP 4: Create SetupComplete.cmd with EC2 commands AND SPEL cleanup -----",
      "Write-Host 'Step 4: Creating SetupComplete.cmd with EC2 and SPEL commands...'",
      "$setupCmd = 'C:\\Windows\\Setup\\Scripts\\SetupComplete.cmd'",
      "New-Item -Path 'C:\\Windows\\Setup\\Scripts' -ItemType Directory -Force | Out-Null",
      "",
      "# Create SetupComplete.cmd with both EC2Launch standard commands and SPEL cleanup",
      "$setupContent = @(",
      "    '@ECHO OFF'",
      "    'echo [%date% %time%] Starting SetupComplete.cmd >> C:\\SetupComplete.log'",
      "    ''",
      "    ':: EC2Launch standard commands'",
      "    'netsh advfirewall firewall set rule group=\"@FirewallAPI.dll,-28752\" new enable=yes'",
      "    ''",
      "    ':: Clean up unattend files'",
      "    'TYPE nul > c:\\windows\\system32\\sysprep\\unattend.xml'",
      "    'del /q /f c:\\windows\\system32\\sysprep\\unattend.xml 2>nul'",
      "    'TYPE nul > c:\\windows\\panther\\unattend.xml'",
      "    'del /q /f c:\\windows\\panther\\unattend.xml 2>nul'",
      "    'TYPE nul > c:\\windows\\panther\\unattend\\unattend.xml'",
      "    'del /q /f c:\\windows\\panther\\unattend\\unattend.xml 2>nul'",
      "    ''",
      "    ':: =============================================='",
      "    ':: SPEL Custom Cleanup'",
      "    ':: =============================================='",
      ...
      ")",
      "$setupContent | Out-File -FilePath $setupCmd -Encoding ASCII",
      "",
      "Write-Host 'SetupComplete.cmd created with EC2 and SPEL commands.'",
      "Write-Host 'Contents of SetupComplete.cmd:'",
      "Get-Content $setupCmd | ForEach-Object { Write-Host \"  $_\" }",
      "",
      "# ----- STEP 5: Run Sysprep directly (not ec2launch sysprep which would overwrite SetupComplete.cmd) -----",
      "Write-Host 'Step 5: Running Sysprep to generalize instance...'",
      "Write-Host 'Instance will shutdown after sysprep completes.'",
      "",
      "# Run sysprep with generalize, OOBE, and shutdown",
