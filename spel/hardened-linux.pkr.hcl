...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo '>> Updating boot= UUID to the device that actually contains /boot/vmlinuz (persistent edit of /etc/default/grub)'",
      "KVER=$(uname -r)",
      "KPATH=\"/boot/vmlinuz-$KVER\"",
      "if [ ! -e \"$KPATH\" ]; then KPATH=$(ls -1 /boot/vmlinuz-* 2>/dev/null | tail -n1 || true); fi",
      "if [ -z \"$KPATH\" ]; then",
      "  echo \"WARNING: couldn't find any /boot/vmlinuz-*; skipping boot= UUID update\"",
      "else",
      "  DEV=$(df --output=source \"$KPATH\" 2>/dev/null | tail -n1)",
      "  BOOT_UUID=$(blkid -s UUID -o value \"$DEV\" 2>/dev/null || true)",
      "  if [ -n \"$BOOT_UUID\" ]; then",
      "    echo \"Setting boot=UUID=$BOOT_UUID (device $DEV contains $KPATH)\"",
      "    grubby --update-kernel=ALL --remove-args=\"boot\" --args=\"boot=UUID=$BOOT_UUID\" || true",
      "    # Backup /etc/default/grub with timestamped extension before editing",
      "    TS=$(date +%Y%m%d%H%M%S)",
      "    cp -a /etc/default/grub /etc/default/grub.bak.$TS || true",
      "    # Safely inject/update boot=UUID into GRUB_CMDLINE_LINUX (best-effort sed)",
      ...
      "  else",
      "    echo \"WARNING: couldn't determine UUID for device $DEV; skipping boot= update\"",
      "  fi",
