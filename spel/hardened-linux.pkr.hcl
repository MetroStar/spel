...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "cp -a /boot/.vmlinuz*.hmac /boot/vmlinuz-$(uname -r).hmac 2>/dev/null || sha512hmac /boot/vmlinuz-$(uname -r) | awk '{print $1}' > /boot/.vmlinuz-$(uname -r).hmac",
      "dracut -f -v --add fips --add-drivers xfs --kver $(uname -r)",
