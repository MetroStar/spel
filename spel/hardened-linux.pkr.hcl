...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "yum install -y python39 python39-pip python39-devel libselinux-python3",
      "python3.9 --version",
      "echo 'Installing SELinux Python bindings and policy management tools for Python 3.9...'",
      "python3.9 -m pip install --upgrade pip",
      "python3.9 -m pip install selinux",
      "echo 'Verifying SELinux module availability...'",
      "python3.9 -c 'import selinux; print(\"selinux module:\", selinux.__file__)' || echo 'selinux import failed'",
      "python3.9 -c 'import sys; print(\"Python path:\"); [print(p) for p in sys.path]'",
      "echo 'Checking for offline Python wheels...'",
      "if [ -d '/tmp/python-deps' ]; then echo '  /tmp/python-deps directory exists'; ls -lh /tmp/python-deps/ | head -5; else echo '  /tmp/python-deps directory NOT found'; fi",
      "if [ -d '/tmp/python-deps' ] && [ \"$(ls -A /tmp/python-deps 2>/dev/null)\" ]; then echo 'Installing Ansible from offline wheels...'; python3.9 -m pip install --no-index /tmp/python-deps/*.whl; else echo 'Installing Ansible from PyPI...'; python3.9 -m pip install ansible-core; fi",
      "export PATH=/usr/local/bin:$PATH",
