...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "if [ -d '/tmp/python-deps' ] && [ \"$(ls -A /tmp/python-deps 2>/dev/null)\" ]; then echo 'Installing Ansible from offline wheels...'; python3 -m pip install --no-index --find-links=/tmp/python-deps ansible-core; else echo 'Installing Ansible from PyPI...'; python3 -m pip install ansible-core; fi",
