...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Creating symlinks for SELinux policy management modules...'",
      "mkdir -p /usr/local/lib64/python3.9/site-packages",
      "for pyfile in /usr/lib/python3.6/site-packages/*.py; do [ -f \"$pyfile\" ] && ln -sf \"$pyfile\" /usr/local/lib64/python3.9/site-packages/; done",
      "for pydir in /usr/lib/python3.6/site-packages/*/; do [ -d \"$pydir\" ] && dirname=$(basename \"$pydir\") && ln -sf \"$pydir\" /usr/local/lib64/python3.9/site-packages/\"$dirname\"; done",
