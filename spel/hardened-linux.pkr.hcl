...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo '>> Determining kernel /boot kernel image and target device'",
      "KVER=$(uname -r || true)",
      "KPATH=\"/boot/vmlinuz-$KVER\"",
      "if [ ! -e \"$KPATH\" ]; then",
      "  KPATH=$(ls -1 /boot/vmlinuz-* 2>/dev/null | tail -n1 || true)",
      "fi",
      "if [ -z \"$KPATH\" ]; then",
      "  echo \"WARNING: cannot find /boot/vmlinuz-*; skipping boot UUID persistence and only using grubby regenerate\"",
      "else",
      "  # Get the block device containing the kernel image (works for normal partitions and device-mapper too)",
      "  DEV=$(df --output=source \"$KPATH\" 2>/dev/null | tail -n1 || true)",
      "  if [ -z \"$DEV\" ]; then",
      "    echo \"WARNING: df couldn't determine device for $KPATH; attempting lsblk fallback\"",
      "    # If KPATH is a file path, find the containing block device via stat + lsblk",
      "    FILEDEV=$(readlink -f \"$KPATH\" 2>/dev/null || true)",
      "    DEV=$(lsblk -no NAME \"$${FILEDEV}\" 2>/dev/null || true)",
      "  fi",
      "  # Resolve to device node if we got a short name",
      "  if [ -n \"$DEV\" ] && [ ! -b \"$DEV\" ]; then",
      "    # try prefixing /dev/",
      "    if [ -b \"/dev/$DEV\" ]; then DEV=\"/dev/$DEV\"; fi",
      "  fi",
      "  BOOT_UUID=$(blkid -s UUID -o value \"$DEV\" 2>/dev/null || true)",
      "  if [ -n \"$BOOT_UUID\" ]; then",
      "    echo \"Found BOOT UUID: $BOOT_UUID for device: $DEV\"",
      "    echo \"Applying grubby kernel arg update (non-persistent kernel args)\"",
      "    grubby --update-kernel=ALL --remove-args=\"boot\" --args=\"boot=UUID=$BOOT_UUID\" || true",
      "    # backup /etc/default/grub before making persistent edit",
      "    TS=$(date +%Y%m%d%H%M%S)",
      "    cp -a /etc/default/grub /etc/default/grub.bak.$TS || true",
      "    # Only write to /etc/default/grub when BOOT_UUID is non-empty (safe sed)",
      "    sed -E -i -e 's@(^[[:space:]]*GRUB_CMDLINE_LINUX=\"[^\"]*)[[:space:]]*boot=[^\" ]*([^\"]*\".*)@\\1 boot=UUID=$BOOT_UUID \\2@' -e 't' -e 's@(^[[:space:]]*GRUB_CMDLINE_LINUX=\"[^\"]*)\"$@\\1 boot=UUID=$BOOT_UUID\"@' /etc/default/grub || true",
      "  else",
      "    echo \"INFO: no BOOT UUID found for device '$DEV' — SKIPPING persistent /etc/default/grub edit to avoid writing empty boot= value\"",
      "    echo \"Applying grubby kernel arg update only (if possible)\"",
      "    grubby --update-kernel=ALL --remove-args=\"boot\" --args=\"boot=UUID=$BOOT_UUID\" || true",
      "  fi",
      "  # Ensure we write grub.cfg to the correct EFI vendor path (best-effort detection)",
      "  if [ -d /boot/efi/EFI/redhat ]; then",
      "    EFI_TARGET=/boot/efi/EFI/redhat/grub.cfg",
      "  elif [ -d /boot/efi/EFI/ol ]; then",
      "    EFI_TARGET=/boot/efi/EFI/ol/grub.cfg",
      "  elif [ -d /boot/efi/EFI/OracleLinux ]; then",
      "    EFI_TARGET=/boot/efi/EFI/OracleLinux/grub.cfg",
      "  elif [ -d /boot/efi/EFI/centos ]; then",
      "    EFI_TARGET=/boot/efi/EFI/centos/grub.cfg",
      "  else",
      "    EFI_TARGET=/boot/grub2/grub.cfg",
      "  fi",
      "  echo \"Generating grub config at $EFI_TARGET\"",
      "  grub2-mkconfig -o \"$EFI_TARGET\" || { echo \"ERROR: grub2-mkconfig failed for $EFI_TARGET\"; }",
      "fi",
      "",
      "# Optional: check for FIPS .hmac presence and warn (do NOT auto-fix in AMI build).",
      "if [ -n \"$KPATH\" ]; then",
      "  HMFILE=\"/boot/.$(basename $KPATH).hmac\"",
      "  if [ ! -e \"$HMFILE\" ]; then",
      "    echo \"WARNING: FIPS .hmac file not found: $HMFILE — if building with fips=1, the AMI may fail boot integrity checks. Do not proceed to enable fips in production images unless you ensure .hmac files are created.\"",
      "  fi",
      "fi",
      "",
      "rm -rf /var/lib/cloud/seed/nocloud-net || true",
      "rm -rf /var/lib/cloud/sem || true",
      "rm -rf /var/lib/cloud/data || true",
  ...
  provisioner "shell" {
    ...
    execute_command = "sudo -E bash '{{.Path}}'"
    inline = [
      "echo 'Running Ansible Lockdown (OL8 variant)'",
      "python3 -m pip install ansible",
      "export PATH=/usr/local/bin:$PATH",
      "yum install -y git",
      "ansible-galaxy install git+https://github.com/ansible-lockdown/RHEL8-STIG.git",
      ...
      "  HMFILE=\"/boot/.$(basename $KPATH).hmac\"",
      "  if [ ! -e \"$HMFILE\" ]; then",
      "    echo \"WARNING: FIPS .hmac file not found: $HMFILE — if building with fips=1, the AMI may fail boot integrity checks.\"",
      "  fi",
      "fi",
      "",
      "rm -rf /var/lib/cloud/seed/nocloud-net || true",
