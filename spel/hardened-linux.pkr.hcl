...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Fixing boot UUID for FIPS mode...'",
      "# Check if /boot is a separate partition or just a directory on root",
      "BOOT_DEV=$(blkid -L boot_disk 2>/dev/null || true)",
      "if [ -n \"$BOOT_DEV\" ]; then",
      "  echo 'Found separate /boot partition: '$BOOT_DEV",
      "  BOOT_UUID=$(blkid -s UUID -o value $BOOT_DEV)",
      "else",
      "  echo '/boot is a directory on root volume, using root device'",
      "  # Find the device mounted at / (root)",
      "  ROOT_DEV=$(findmnt -n -o SOURCE /)",
      "  echo 'Root device: '$ROOT_DEV",
      "  BOOT_UUID=$(blkid -s UUID -o value $ROOT_DEV)",
      "fi",
