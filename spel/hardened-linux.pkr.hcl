...
build {
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "echo 'Running Ansible Lockdown'",
      "echo 'Using system Python 3.6 for EL8 (has SELinux bindings)...'",
      "python3.6 --version",
      "echo 'Upgrading pip for Python 3.6...'",
      "python3.6 -m pip install --upgrade pip",
      "echo 'Checking for offline Python wheels...'",
      "if [ -d '/tmp/python-deps' ]; then echo '  /tmp/python-deps directory exists'; ls -lh /tmp/python-deps/ | head -5; else echo '  /tmp/python-deps directory NOT found'; fi",
      "if [ -d '/tmp/python-deps' ] && [ \"$(ls -A /tmp/python-deps 2>/dev/null)\" ]; then echo 'Installing Ansible from offline packages...'; python3.6 -m pip install --no-index --find-links /tmp/python-deps/ 'ansible>=4.0.0,<5.0.0'; else echo 'Installing Ansible from PyPI...'; python3.6 -m pip install 'ansible>=4.0.0,<5.0.0'; fi",
      "export PATH=/usr/local/bin:$PATH",
      "echo 'Installing Ansible collections...'",
      "if [ -d '/tmp/ansible-collections' ]; then for tarball in /tmp/ansible-collections/*.tar.gz; do [ -f \"$tarball\" ] && ansible-galaxy collection install \"$tarball\" --force; done; fi",
      "mkdir -p $HOME/.ansible/roles",
  ...
  provisioner "shell" {
    ...
    inline = [
      ...
      "mkdir -p $HOME/.ansible/roles",
      "cp -r /tmp/RHEL8-STIG $HOME/.ansible/roles/",
      "ansible-playbook -vvv -i localhost, -c local $HOME/.ansible/roles/RHEL8-STIG/site.yml -e '{\"system_is_ec2\": true, \"rhel8stig_copy_existing_zone\": false, \"setup_audit\": true, \"run_audit\": true, \"fetch_audit_output\": true, \"rhel_08_040136\":false}'",
      "bash /tmp/boot-fips-wrapper.sh post",
      "rm -rf /var/lib/cloud/seed/nocloud-net",
      "rm -rf /var/lib/cloud/sem",
      "rm -rf /var/lib/cloud/data",
