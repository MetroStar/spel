...
##############################################################################
PROGNAME="$(basename "$0")"
AMIGENBOOTDEVMULT="${GRANITE_AMIGENBOOTDEVMULT:-1.2}"
AMIGENBOOTDEVSZ="${GRANITE_AMIGENBOOTDEVSZ:-1024}"
AMIGENBOOTSIZE="${GRANITE_AMIGENBOOTSIZE}"
AMIGENBRANCH="${GRANITE_AMIGENBRANCH:-master}"
AMIGENCHROOT="${GRANITE_AMIGENCHROOT:-/mnt/ec2-root}"
AMIGENCROSSDISTRO="${GRANITE_AMIGENCROSSDISTRO:-false}"
AMIGENNOSIGNATURE="${GRANITE_AMIGENNOSIGNATURE:-false}"
AMIGENFSTYPE="${GRANITE_AMIGENFSTYPE:-xfs}"
AMIGENICNCTURL="${GRANITE_AMIGENICNCTURL}"
AMIGENMANFST="${GRANITE_AMIGENMANFST}"
AMIGENPKGGRP="${GRANITE_AMIGENPKGGRP:-core}"
AMIGENREPOS="${GRANITE_AMIGENREPOS}"
AMIGENREPOSRC="${GRANITE_AMIGENREPOSRC}"
AMIGENROOTNM="${GRANITE_AMIGENROOTNM}"
AMIGENSOURCE="${GRANITE_AMIGEN8SOURCE:-file://$(dirname $(dirname $(dirname $(readlink -f "$0"))))/vendor/amigen8}"
AMIGENSSMAGENT="${GRANITE_AMIGENSSMAGENT}"
AMIGENSTORLAY="${GRANITE_AMIGENSTORLAY}"
AMIGENTIMEZONE="${GRANITE_TIMEZONE:-UTC}"
AMIGENVGNAME="${GRANITE_AMIGENVGNAME}"
AWSCFNBOOTSTRAP="${GRANITE_AWSCFNBOOTSTRAP}"
AWSCLIV1SOURCE="${GRANITE_AWSCLIV1SOURCE}"
AWSCLIV2SOURCE="${GRANITE_AWSCLIV2SOURCE:-https://awscli.amazonaws.com/awscli-exe-linux-x86_64.zip}"
CLOUDPROVIDER="${GRANITE_CLOUDPROVIDER:-aws}"
EPELRELEASE="${GRANITE_EPELRELEASE}"
EPELREPO="${GRANITE_EPELREPO}"
EXTRARPMS="${GRANITE_EXTRARPMS}"
FIPSDISABLE="${GRANITE_FIPSDISABLE}"
GRUBTMOUT="${GRANITE_GRUBTMOUT:-5}"
HTTP_PROXY="${GRANITE_HTTP_PROXY}"
USEDEFAULTREPOS="${GRANITE_USEDEFAULTREPOS:-true}"
USEROOTDEVICE="${GRANITE_USEROOTDEVICE:-true}"
AMIGENSSLVERIFY="${GRANITE_AMIGENSSLVERIFY:-true}"


read -r -a BUILDDEPS <<< "${GRANITE_BUILDDEPS:-lvm2 yum-utils unzip git}"

ELBUILD="/tmp/el-build"

# Debug: Show air-gapped build configuration
echo "=== Air-Gapped Build Configuration ==="
echo "GRANITE_AMIGENCROSSDISTRO=${GRANITE_AMIGENCROSSDISTRO:-not set}"
echo "AMIGENCROSSDISTRO=${AMIGENCROSSDISTRO}"
echo "GRANITE_AMIGENNOSIGNATURE=${GRANITE_AMIGENNOSIGNATURE:-not set}"
echo "AMIGENNOSIGNATURE=${AMIGENNOSIGNATURE}"
echo "GRANITE_USEDEFAULTREPOS=${GRANITE_USEDEFAULTREPOS:-not set}"
echo "GRANITE_AMIGENREPOSRC=${GRANITE_AMIGENREPOSRC:-not set}"
echo "GRANITE_AMIGENREPOS=${GRANITE_AMIGENREPOS:-not set}"
echo "GRANITE_AMIGENSSLVERIFY=${GRANITE_AMIGENSSLVERIFY:-not set}"
...
function PrepBuildDevice {
    ...
    if [[ -n ${GRANITE_AMIGENBUILDDEV:-} ]]
    then
      AMIGENBUILDDEV="${GRANITE_AMIGENBUILDDEV}"
    else
...
echo "Checking ${GRANITE_AMIGENBUILDDEV} for VTOC to nuke..."
if [[ -b "${GRANITE_AMIGENBUILDDEV}" ]]
then
    echo "${GRANITE_AMIGENBUILDDEV} is a valid block device. Nuking VTOC... "

    retry 5 dd if=/dev/urandom of="${GRANITE_AMIGENBUILDDEV}" bs=1024 count=10240
    echo "Cleared."
fi
