...
# =============================================================================
# Automatically propagates StigPlatform and StigManaged tags from Chimera AMIs
...
# A Lambda function checks the instance's source AMI for Chimera tags
...
def handler(event, context):
    ...
    try:
        ...
        # Only tag instances launched from Chimera AMIs (StigManaged=true)
        if ami_tags.get('StigManaged') != 'true':
            logger.info("Instance %s not from Chimera AMI (AMI %s)", instance_id, image_id)
