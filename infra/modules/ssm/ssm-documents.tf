    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "",
            "# ----- Verify Python is pre-installed (baked into AMI) -----",
            "Write-Host '=== Verifying Python and Ansible ==='",
            "# Refresh PATH to ensure Python install directories are included",
            "$env:PATH = [System.Environment]::GetEnvironmentVariable('PATH', 'Machine') + ';' + [System.Environment]::GetEnvironmentVariable('PATH', 'User')",
            "",
            "if (-not (Get-Command python -ErrorAction SilentlyContinue)) {",
            "  Write-Host 'ERROR: Python is not installed on this instance.'",
            "  Write-Host 'Python and ansible-core must be baked into the AMI during the Packer build.'",
            "  Write-Host 'Ensure the Python provisioner in hardened-linux.pkr.hcl ran successfully.'",
            "  exit 1",
            "}",
            ...
            "}",
            "if (-not $ansiblePath) {",
            "  Write-Host 'ERROR: ansible-playbook not found. Ensure ansible-core is installed in the AMI.'",
            "  exit 1",
            "}",
