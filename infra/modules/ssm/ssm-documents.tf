    ...
    mainSteps = [
      {
        ...
        inputs = {
          ...
          runCommand = [
            ...
            "",
            "# ----- Helper: ensure Python is installed -----",
            "Write-Host '=== Ensuring Python is available ==='",
            "if (-not (Get-Command python -ErrorAction SilentlyContinue)) {",
            "  Write-Host 'Python not found — installing via winget or direct download...'",
            "  # Try winget first (available on newer Windows Server)",
            "  $installed = $false",
            "  if (Get-Command winget -ErrorAction SilentlyContinue) {",
            "    winget install --id Python.Python.3.11 --accept-package-agreements --accept-source-agreements --scope machine 2>&1 | Write-Host",
            "    # Refresh PATH",
            "    $env:PATH = [System.Environment]::GetEnvironmentVariable('PATH', 'Machine') + ';' + [System.Environment]::GetEnvironmentVariable('PATH', 'User')",
            "    if (Get-Command python -ErrorAction SilentlyContinue) { $installed = $true }",
            "  }",
            "  if (-not $installed) {",
            "    Write-Host 'winget unavailable or failed — downloading Python installer...'",
            ...
            "  }",
            "  if (-not (Get-Command python -ErrorAction SilentlyContinue)) {",
            "    Write-Host 'ERROR: Failed to install Python.'",
            "    exit 1",
            "  }",
            "}",
